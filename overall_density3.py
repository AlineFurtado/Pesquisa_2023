...
# Modifiquei a função boundary_estimate para retornar a região estimada da zona social
    ...
    def is_point_in_field_of_vision(self, person, point_x, point_y):
        ...
        upper_limit = person.th + np.deg2rad(90)
                ...
                for col in range(cluster.shape[1]):
                    if cluster[row, col] == 1:  # Verifique se o ponto pertence ao cluster
                        ...
                        if self.is_point_in_field_of_vision(person, point_x, point_y):
                            ...
                            print('ponto fora do fov')

        return points_in_fov


#    def find_contour_points(self, cluster_coords):
        # cluster_coords é uma lista de coordenadas do cluster
    
        # Crie uma matriz booleana para representar o cluster
#        cluster_matrix = np.zeros((self.map_resolution, self.map_resolution), dtype=bool)
    
#        for coord in cluster_coords:
#            row, col = coord
#            cluster_matrix[row, col] = True
    
#        # Use a função find_contours do scipy para encontrar o contorno do cluster
#        contours = measure.find_contours(cluster_matrix, 0.5)
    
#        # A função find_contours retorna uma lista de contornos, onde cada contorno é uma lista de coordenadas (row, col)
#        # Vamos usar o primeiro contorno encontrado
#        if contours:
#            contour = contours[0]
#            return [(self.x[int(col)], self.y[int(row)]) for row, col in contour]
#        else:
#            return []  # Se nenhum contorno for encontrado, retorne uma lista vazia
