...
#falta associar peso, pontos nessa região receberão peso mediun (por ex:3)
    def is_approach_point(self, person, point_x, point_y):
        ...
        # Calcule os limites do campo de visão em radianos
        lower_limit = - np.deg2rad(45)
        upper_limit =  np.deg2rad(45)
...
#verifica se uma f-formation é aproximandamente um triângulo equilátero
    ...
    def calculate_zone_points(self, center, distance, angle, start_angle, step_angle):
        zone_points = []
        current_angle = start_angle

        while current_angle < start_angle + 2 * np.pi:
            x = center[0] + distance * np.cos(current_angle)
            y = center[1] + distance * np.sin(current_angle)
            zone_points.append((x, y))
            current_angle += step_angle

        return zone_points

    #verifica se o ponto esté nas costas de alguma das pessoas no cluster
    ...
    #    for person in people:
    #        print(f'pessoa:{person.x},{person.y},{np.rad2deg(person.th)}')
    #        print(f'point:{point}')
    #        delta_x = point_x - person.x
    #        delta_y = point_y - person.y
    ...
    #        # Ajuste para levar em consideração a orientação da pessoa
    #        person_th_normalized = (person.th + math.pi) % (2 * math.pi) - math.pi
    #        print(f'person.th_norm:{np.rad2deg(person_th_normalized)}')
            
            # Calcular o ângulo em relação à orientação da pessoa
    #        angle_to_point = math.atan2(delta_y, delta_x) - person_th_normalized
    #        print(f'angle:{np.rad2deg(angle_to_point)}')
    #        angle_to_point = (angle_to_point + 2 * math.pi) % (2 * math.pi)
    #        print(f'angle_norm:{np.rad2deg(angle_to_point)}')
    ...
    #        print(f'angle_opost:{np.rad2deg(opposite_angle)}')
    #        if (opposite_angle - math.pi / 3) <= angle_to_point <= (opposite_angle + math.pi / 3):
    #            print('angulo está nas costas da pessoa')
    #            is_back = True
    #            break  # Se estiver nas costas de uma pessoa, não é necessário verificar as outras
    #        print('angulo não está nas costas')
    #    return is_back
    def is_point_back(self, people, point):
        point_x, point_y = point
        is_back = False
                            ...
                            for col in range(self.cluster[cluster_idx].shape[1]):
                                if self.cluster[cluster_idx][row, col] == 1:  # Verifique se o ponto pertence ao cluster
                                    ...
                                    if self.is_better_approach_point(person, point_x, point_y):
                                        approach_points[person].remove((point_x, point_y))
                                        better_approach_points[person].append((point_x, point_y))
                #generalizar a regra de 2 pessoas para 3 e 4 pessoas
                else:
                            ...
                            if angulo1 != angulo2:
                                ...
                                if diferenca == np.deg2rad(180) or diferenca == np.deg2rad(0):
                                    if mean_orientation == np.deg2rad(0) or mean_orientation == np.deg2rad(180): #caso específico que ocorre com 4 pessoas
                                        ...
                                        p_Ocenter1 = Person(xc, yc, mean_orientation, id_node=0)
                                        p_Ocenter2 = Person(xc, yc, mean_orientation_simetrico, id_node=1)
                                        for row in range(self.cluster[cluster_idx].shape[0]):
                                            for col in range(self.cluster[cluster_idx].shape[1]):
                                                if self.cluster[cluster_idx][row, col] == 1:  # Verifique se o ponto pertence ao cluster
                                                    point_x = self.x[col]
